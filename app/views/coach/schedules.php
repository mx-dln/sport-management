...
<form class="mb-6 grid gap-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-4" method="post" action="<?= project_url('app/ajax/schedule_ajax.php') ?>" data-ajax-form><select class="form-input" name="sport_id" required><option value="">Sport</option><?php foreach ($sports as $s): ?><option value="<?= e($s['id']) ?>"><?= e($s['name']) ?></option><?php endforeach; ?></select><select class="form-input" name="team_id" required><option value="">Team</option><?php foreach ($teams as $t): ?><option value="<?= e($t['id']) ?>"><?= e($t['name']) ?></option><?php endforeach; ?></select><input type="hidden" name="coach_id" value="<?= e($uid) ?>"><input class="form-input" type="date" name="training_date" required><input class="form-input" type="time" name="start_time" required><input class="form-input" type="time" name="end_time" required><input class="form-input" name="venue" placeholder="Venue" required><select class="form-input" name="status"><option>Scheduled</option><option>Updated</option><option>Cancelled</option><option>Completed</option></select><label class="flex items-center gap-2 text-sm"><input type="checkbox" name="send_sms"> Send SMS</label><textarea class="form-input md:col-span-4" name="description" placeholder="Description"></textarea><button class="btn-primary md:col-span-4">Save Schedule</button></form>
<div class="grid gap-4 md:grid-cols-2"><?php foreach ($schedules as $s): ?><article class="rounded-xl bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><h3 class="font-bold"><?= e($s['team_name']) ?></h3><span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700"><?= e($s['status']) ?></span></div><p class="mt-1 text-sm"><?= e($s['sport_name']) ?> - <?= e($s['coach_name']) ?></p><p class="mt-2 text-sm text-slate-600"><?= e($s['training_date'].' '.format_time_range($s['start_time'] ?? '', $s['end_time'] ?? '').' @ '.$s['venue']) ?></p><?php if ($s['description']): ?><p class="mt-2 text-sm text-slate-500"><?= e($s['description']) ?></p><?php endif; ?><div class="mt-4 flex gap-2"><button class="rounded-lg border border-slate-300 px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50" type="button" data-modal-open="#coach-schedule-edit-modal-<?= e((string)$s['id']) ?>">Edit</button><form method="post" action="<?= project_url('app/ajax/schedule_ajax.php') ?>" data-ajax-form data-confirm="Delete this training schedule and its attendance records?"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= e((string)$s['id']) ?>"><button class="rounded-lg border border-rose-200 px-3 py-2 text-sm font-bold text-rose-600 hover:bg-rose-50" type="submit">Delete</button></form></div></article><?php endforeach; ?></div>

<?php foreach ($schedules as $s): ?>
<div id="coach-schedule-edit-modal-<?= e((string)$s['id']) ?>" class="fixed inset-0 z-[70] hidden bg-slate-950/60 p-4 backdrop-blur-sm" data-modal>
    <div class="mx-auto flex min-h-full max-w-2xl items-center">
        <section class="max-h-[86vh] w-full overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <form class="grid max-h-[70vh] gap-3 overflow-y-auto p-5 md:grid-cols-2" method="post" action="<?= project_url('app/ajax/schedule_ajax.php') ?>" data-ajax-form data-validate>
                ...
                <input type="hidden" name="coach_id" value="<?= e((string)$s['coach_id']) ?>">
                <label class="block">
